  ...
  $(function () {
    ...
    function sv_disper_bar() {
      $.post(localize.ajax_url, sv_disper_bar_data, function (response) {
        ...
        if (sv_counter > 0) {
          $("#sv-disper-link-text").text(
            localize.text_yas + ": " + sv_counter + "%"
          );
        } else {
    ...
    $(document.body).on(
      "added_to_cart updated_wc_div removed_from_cart updated_cart_totals updated_checkout updated_shipping_method wc_fragment_refresh wc_fragments_refreshed wc_fragments_loaded",
      sv_disper_bar
    );
  });
});

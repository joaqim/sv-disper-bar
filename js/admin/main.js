jQuery(function ($) {
  ...
  $(function () {
    ...
    $("#sv-disper-add").on("click", function () {
      ...
      html +=
        '<input class="sv-disper-price code" type="text" onkeypress="sv_number_validate(event)" onkeyup="this.value = this.value.replace(/[^0-9\\.]/g, \'\')" name="sv-disper-bar-points[rules][' +
        key +
        '][price]">';
      html +=
        ' <input class="sv-disper-percent code" type="text" onkeypress="sv_number_validate(event)" onkeyup="this.value = this.value.replace(/[^0-9\\.]/g, \'\')" name="sv-disper-bar-points[rules][' +
        key +
        '][percent]">';
      html +=
        ' <button id="sv-disper-remove" class="button button-secondary" type="button">' +
        localize.sv_delete +
        "</button>";
      html += "</td>";
      html += "</tr>";
        ...
        change: function (event, ui) {
          switch (this.name) {
            case sv_disper_bar_prefix_name + "[container-background]":
              return sv_disper_container.css(
                "background-color",
                ui.color.toString()
              );
            ...
            case sv_disper_bar_prefix_name + "[price-title-border]":
              return sv_disper_data_title.css(
                "border-color",
                ui.color.toString()
              );
    ...
    $(".sv-disper-colors-box .wp-picker-default").val(
      sv_disper_bar_params.sv_reset
    );
  });
});
